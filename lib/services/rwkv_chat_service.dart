...
class RWKVChatService extends GetxController {
  ...
  void _setupReceivePortListener() {
    RoleplayManage.receivePort?.listen((message) async {
  ...
  void operationChatMessage(dynamic message) async {
    ...
    if (message is SendPort) {
      ...
      // int? tmpModelID = message.modelID;
      // if (tmpModelID != null && tmpModelID >= 0) {
      //   modelID = tmpModelID;
      //   initWork();
      // }
      // debugPrint('LoadModelSteps message==${message.modelID}');
    } else {
      if (message is ResponseBufferContent) {
        ...
        if (!generating && isNeedSaveAiMessage) {
          ...
          debugPrint(
            '11modelpath==${CommonUtil.getFileName(chatmodelPath.value)},statepath=${CommonUtil.getFileName(statePath.value)},backend=${backend.toString().split('.').last}',
          );
    ...
    setGlobalModelDownloadCompleteCallback((ModelInfo? info) async {
      ...
      if (info != null) {
        if (info.modelType == RoleplayManageModelType.chat) {
          ...
          ttsmodelPath.value = await CommonUtil.getFileDocumentPath(
            info.modelPath,
          );
        }
      }
  ...
  Future<void> loadChatModel({ModelInfo? info}) async {
    ...
    debugPrint(
      'loadChatModel， backend: $backend, modelPath: $chatmodelPath, statePath: $statePath',
    );
  ...
  void initWork() async {
    ...
    debugPrint(
      '22modelpath==${CommonUtil.getFileName(chatmodelPath.value)},statepath=${CommonUtil.getFileName(statePath.value)},backend=${backend.toString().split('.').last}',
    );

    if (rmpack != null && rmpack!.isNotEmpty) {
      if (!isHiddenState) {
        send(
          to_rwkv.LoadInitialStates(
            rmpack!,
            modelID: RoleplayManage.chatModelID,
          ),
        );
  ...
  void _setupModelParameters() {
    ...
    send(to_rwkv.SetPrompt(prompt, modelID: RoleplayManage.chatModelID));
  ...
  /// 清空状态
  Future<void> clearStates({String? tmpstatePath}) async {
    // prefillSpeed.value = 0;
    // decodeSpeed.value = 0;
    ...
    debugPrint('加载角色缓存state: $stateLoadPath');
    debugPrint(
      '33modelpath==${CommonUtil.getFileName(chatmodelPath.value)},statepath=${CommonUtil.getFileName(statePath.value)},backend=${backend.toString().split('.').last}',
    );
  ...
  Future<void> generate(String prompt) async {
    ...
    _getTokensTimer = Timer.periodic(const Duration(milliseconds: 100), (
      ...
      send(
        to_rwkv.GetPrefillAndDecodeSpeed(modelID: RoleplayManage.chatModelID),
      );
      send(
      ...
      await Future.delayed(const Duration(milliseconds: 1000));
      send(to_rwkv.GetIsGenerating(modelID: RoleplayManage.chatModelID));
